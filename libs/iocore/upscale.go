...
func (u *runpodUpscaler) Upscale(ctx context.Context, r io.Reader, w io.Writer) (time.Duration, error) {
	...
	if u.config.UseVolume {
		...
		tmpOutDir, err := os.MkdirTemp("", "io-upscale-out-*")
		if err != nil {
			return 0, err
		}
		...
		if len(u.config.DataCenterIDs) > 0 {
			volWorkflowCfg.Region = u.config.DataCenterIDs[0]
		} else {
		...
		if len(files) == 0 {
			return 0, fmt.Errorf("no output file generated by volume workflow")
		}
		outData, err := os.ReadFile(filepath.Join(tmpOutDir, files[0].Name()))
		if err != nil {
			return 0, err
		}
	...
	// 2. Fallback to Base64 workflow
	var buf bytes.Buffer
	if _, err := io.Copy(&buf, r); err != nil {
		return 0, err
	}

	// 3. Submit via /runsync — blocks until result is ready, zero polling overhead
